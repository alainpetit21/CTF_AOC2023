...
# When a new number is found, it is added to a list hat will act as a queue for later processing
def add_number_to_processing_queue(value: int):
    processing_gear_queue.append(value)


def process_queue() -> int:

    sum_num = 0
    for number in processing_gear_queue:
        sum_num += number
...
def main():
    ...
    for coord_symbol in symbols_scanner():
        ...
        for coord_digit in kernel_processor(coord_symbol[0], coord_symbol[1]):
            ...
            set_number.add(coord_start_numb)

        powers_value = 1
        if len(set_number) == 2:
            for coord in set_number:
                number, _ = detect_whole_number(coord[0], coord[1])
                powers_value *= number

            add_number_to_processing_queue(powers_value)
